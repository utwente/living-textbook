...
d3sparql.graph = function(json, config) {
    ...
    var opts = {
        ...
        "node1Label": config.node1Label || head[2] || false,
        "node2Label": config.node2Label || head[3] || false,
        "node1Link": config.node1Link || head[4] || false,
        "node2Link": config.node2Link || head[5] || false,
        "relationLabel": config.relationLabel || head[6] || false
    };
    var graph = {
        "nodes": [],
        "links": []
    ...
    for (var i = 0; i < data.length; i++) {
        ...
        if (typeof  data[i][opts.node1Link] == "object") {
            var node1Link = data[i][opts.node1Link].value;
        } else {
            node1Link = "none";
        }

        if (typeof  data[i][opts.node2Link] == "object") {
            var  node2Link = data[i][opts.node2Link].value;
        } else {
            node2Link = "none";
        }
//      var node2Link = data[i][opts.node2Link].value ? data[i][opts.node2Link].value : false;
        ...
        graph.links.push({"source": check.get(node1URI), "target": check.get(node2URI), "relationName": relationLabel})
    }
