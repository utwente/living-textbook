...
class ReviewService
{
  ...
  public function canObjectBeEdited(StudyArea $studyArea, ReviewableInterface $object): bool
  {
    ...
    return 0 === count(array_filter($this->pendingChangeRepository->getForObject($object),
      static fn (PendingChange $pendingChange): bool => $pendingChange->getChangeType() !== PendingChange::CHANGE_TYPE_EDIT));
  }
  ...
  public function publishReview(Review $review): void
  {
    ...
    $this->entityManager->wrapInTransaction(static function (EntityManagerInterface $em): void {
      $em->flush();
    });
