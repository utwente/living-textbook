...
class PermissionsController extends AbstractController
{
    ...
    UserGroupRepository $userGroupRepository, UserRepository $userRepository, TranslatorInterface $trans): Response
  {
    ...
    if ($form->isSubmitted() && $form->isValid()) {
      ...
      $emails     = array_diff($emails, array_map(static fn (User $foundUser): string => $foundUser->getUserIdentifier(), $foundUsers));
    ...
    UserGroupRepository $userGroupRepository): JsonResponse
  {
    ...
    $newPermission   = null;
    $userGroupEmails = $userGroup->getEmails()->filter(static fn (UserGroupEmail $userGroupEmail): bool => $userGroupEmail->getEmail() === $email);
    ...
    EntityManagerInterface $em, UserGroupRepository $userGroupRepository, TranslatorInterface $trans): Response
  {
    ...
    if ($groupType === UserGroup::GROUP_VIEWER) {
      $notNecessary = 0 === count(array_filter($userPermissions, static fn (UserPermissions $userPermission): bool => $userPermission->isViewerOnly()));
    } else {
    ...
    Request $request, RequestStudyArea $requestStudyArea, string $email, EntityManagerInterface $em, TranslatorInterface $trans): Response
  {
    ...
    $userGroupEmails = [];
    foreach ($studyArea->getUserGroups() as $userGroup) {
      $userGroupEmails = array_merge($userGroupEmails, $userGroup->getEmails()->filter(static fn (UserGroupEmail $userGroup): bool => $userGroup->getEmail() == $email)->toArray());
    }
