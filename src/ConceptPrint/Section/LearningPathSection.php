...
class LearningPathSection extends LtbSection
{
    ...
    LearningPath $learningPath, LtbRouter $router, TranslatorInterface $translator, NamingService $namingService, string $projectDir)
  {
    ...
    $concepts = $learningPath->getElementsOrdered()->map(static fn (LearningPathElement $learningPathElement): ?Concept => $learningPathElement->getConcept());

    // Add concept list
    $this->addElement(new SubSection($translator->trans('menu.concept'))
      ->addElement(new Listing($concepts->map(static fn (Concept $concept): string => $concept->getName())->toArray())));
